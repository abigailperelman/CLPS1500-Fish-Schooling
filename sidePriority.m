%% Find fish of influence based on angular distance to the side of the selected fish
...
for fish = 1:numOfFish
    [~, neighborsTotal(fish, :)] = mink(abs(abs(distanceAngle(fish, :)) - 90), numOfFish);
end

% remove last column, fish doesn't count itself
neighborsTotal(:, end) = [];

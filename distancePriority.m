%% Find fish of influence based on distance to selected fish

% initialize array
neighborsTotal = zeros(numOfFish);

% sort a matrix by closest fish -- row = fish, column = ranking, values = indices
for fish = 1:numOfFish
    [~, neighborsTotal(fish, :)] = mink(distanceDirect(fish, :), numOfFish);
end

% remove first column (closest fish will be itself with distance 0)
neighborsTotal(:, 1) = [];
